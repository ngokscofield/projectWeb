...
<link rel="stylesheet" type="text/css" href="<%=request.getContextPath() %>/css/result-listening.css">
<style>
html {
	min-height: 100vh;
}
...
<body>
	...
	ArrayList<Integer> listAnswer = (ArrayList<Integer>) request.getAttribute("answerSelected");	
	int count = 0;				
%>
	<div id="header">
	...
	<div id="main">
		<div class="main-content">
			...
			<div class="result-score-test">
	        <h4>Scored: <%=request.getAttribute("totalScore") %>/495</h4>
	        <p>
	        <b>Listening: </b> đúng <span><%=request.getAttribute("rightAnswer") %></span>/100 câu
	        </p>
	        </div> 
				...
				<div class="right-display">
					...
					<div class="card">
						<div class="card-question card-question-image">
							...
							for(int i = 0; i < 4; i++){
								boolean selected = listAnswer.contains(answers.get(i).getAnswerId());
								int isCorrect = answerDAO.isCorrect(answers.get(i).getAnswerId());
								%>								
								<div class="answer">								
									<input type="radio" name="answerquestion<%=count%>" disabled="disabled" id="" value="<%=answers.get(i).getAnswerId() %>" <%=selected? "checked":"" %>><b><%=character[i] %>. </b> <%=answers.get(i).getContent() %><br> 
								<%if(selected){
									if(isCorrect == 1) {%>
										<img src="<%=request.getContextPath() %>/img/right-answer.jpg">	
									<%}
									else {%>
										<img src="<%=request.getContextPath() %>/img/wrong-answer.jpg">					
									<% }
									}
								%>	
								</div>
